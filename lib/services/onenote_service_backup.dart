...
File path: $filePath''',
  ...
  bool _containsOnlyNoise(String content) {
    ...
    if (potentialPages.length > 1) {
      pages.addAll(potentialPages.where((page) => page.trim().length > 30));
    } else {
      // Fallback to original logic but with smaller chunks
      final sections = content.split(RegExp(r'\n\s*\n\s*\n')); // Triple line breaks
      
      if (sections.length > 1) {
        pages.addAll(sections.where((section) => section.trim().length > 30));
      } else {
        // Split into smaller chunks to find more content
        const maxPageSize = 1000; // Smaller chunks
        if (content.length <= maxPageSize) {
          pages.add(content);
        } else {
          final paragraphs = content.split(RegExp(r'\n\s*\n'));
          String currentPage = '';
          ...
          for (final paragraph in paragraphs) {
            if (currentPage.length + paragraph.length > maxPageSize && currentPage.isNotEmpty) {
