...
class OneNoteService {
  ...
  Future<List<OneNotePage>> readOneNoteFile(String filePath) async {
    ...
    try {
      ...
      // First try COM automation (if OneNote is installed)
      try {
        final comPages = await _extractUsingCOM(filePath);
        if (comPages.isNotEmpty) {
          pages.addAll(comPages);
          print('Successfully extracted ${comPages.length} pages using COM automation');
          return pages;
        }
      } catch (e) {
        print('COM extraction failed: $e, falling back to file parsing');
      }
...
File: $filePath''',
  ...
  Map<String, String> _extractBusinessMetadata(String chunk) {
    ...
    // Extract underwriter
    final underwriterMatch = RegExp(r'underwriter:\s*([A-Za-z\s&,.\''-]+)', caseSensitive: false).firstMatch(chunk);
    if (underwriterMatch != null) {
      metadata['underwriter'] = underwriterMatch.group(1)?.trim() ?? '';
    }
    
    // Extract company
    final companyMatch = RegExp(r'(?:company|business|client|account):\s*([A-Za-z\s&,.\''-]+)', caseSensitive: false).firstMatch(chunk);
    if (companyMatch != null) {
      metadata['company'] = companyMatch.group(1)?.trim() ?? '';
    }
    
    // Extract broker
    final brokerMatch = RegExp(r'broker:\s*([A-Za-z\s&,.\''-]+)', caseSensitive: false).firstMatch(chunk);
    if (brokerMatch != null) {
    ...
    if (metadata.containsKey('underwriter') && metadata['underwriter']!.isNotEmpty && metadata['underwriter']!.toLowerCase() != 'n/a') {
      title = 'Underwriter: ${metadata['underwriter']}';
      metadata['section'] = 'Underwritten Policies';
    } else if (metadata.containsKey('company') && metadata['company']!.isNotEmpty) {
      title = 'Company: ${metadata['company']}';
      metadata['section'] = 'Company Records';
    } else if (metadata.containsKey('broker') && metadata['broker']!.isNotEmpty) {
      title = 'Broker: ${metadata['broker']}';
      metadata['section'] = 'Broker Records';
    }
